...
class ForestLake{
  constructor(bg_forestLake, flowerRed, flowerPink, flowerPurple, flowerYellow, fishingSign, lakeTheme){
  ...
  playTheme() {
    if (scene === "lake") {
      if (!lakeTheme.isPlaying() && !phoneSong1.isPlaying() && !phoneSong2.isPlaying() && !phoneSong3.isPlaying()) {
        lakeTheme.play();
      }
    }
  }
  ...
  friendText1(stateOfText, dialogue){
    if (player.sprite.position.y < 1041 && textStateFL_1 === stateOfText){
  ...
  askToFish(stateOfText, q, c1, c2, positioning){
    if (this.sprite.overlap(player.sprite) && textStateFL_3 === stateOfText && textStateFL_2 > 1){
      choice();
      dynamicTextBox();
      fill(255);
      question = q;
  ...
  changeTextState1(){
    if (friendTalkedToLake1 === true && keyCode === SHIFT && textStateFL_1 < 2){
  ...
  changeTextState2(){
    if (friendTalkedToLake2 === true && keyCode === SHIFT && textStateFL_2 < 2)  {
  ...
  changeTextState4(){
    if (friendTalkedToLake3 === true && keyCode === SHIFT && textStateFL_4 < 6){
  ...
  exit(){
    if(friendTalkedToLake3 === true && player.sprite.position.y < 287 && fishCaught > 4){
      scene = "forestPath3";
      forestPath3.start();
      lakeTheme.stop()
    }
